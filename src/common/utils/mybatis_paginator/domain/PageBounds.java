...
public class PageBounds extends RowBounds implements Serializable {
    ...
    public PageBounds(PagingConfig pagingConfig) {
    	
    	Integer pageNo = Integer.parseInt(StringUtil.defaultIfEmpty(pagingConfig.getPageNo(), "-1"));
		if(pageNo == -1) {
			throw new UserBizException("페이지번호가 존재하지 않습니다.");
		}
		
		List<PagingConfigOrder> orders = pagingConfig.getOrders();
		if(orders == null) {
			throw new UserBizException("페이지 정렬기준이 존재하지 않습니다.");
		}
		
		List<Order> orderlist = new ArrayList<Order>();
		for(PagingConfigOrder order : orders) {
			String target = order.getTarget();
			if(StringUtil.isNotEmpty(target.trim())) {
				orderlist.add(Order.create(order.getTarget(), order.getIsAsc() ? "ASC" : "DESC"));
			}
		}
		
		Integer limit = Integer.parseInt(StringUtil.defaultIfEmpty(pagingConfig.getLimit(), "-1"));
		if(limit == -1) {
