	...
	<div class="main-content">
		...
		<template>
					...
					<div class="panel panel-primary">
						...
						<form class="form-horizontal form-groups-bordered">
							<div class="form-group">
								<label class="control-label" style="float: left;margin-left: 30px;width: 150px;">분류별 검색</label>
								<div style="float: left; margin-left: 20px;">
									<select class="form-control col-sm-1" id="cat_cd_1" v-model="cat_cd_1" style="width: 150px;" @change="changeGbn(event.target)">
										<option value="">- 1차분류 -</option>
										<option :value="item.code" v-for="item in catList1" >{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="cat_cd_2" v-model="cat_cd_2" style="width: 150px;" @change="changeGbn(event.target)">
										<option value="">- 2차분류 -</option>
										<option :value="item.code" v-for="item in catList2">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="cat_cd_3" v-model="cat_cd_3" style="width: 150px;" @change="changeGbn(event.target)">
										<option value="">- 3차분류 -</option>
										<option :value="item.code" v-for="item in catList3">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="cat_cd_4" v-model="cat_cd_4" style="width: 150px;" @change="changeGbn(event.target)">
										<option value="">- 4차분류 -</option>
										<option :value="item.code" v-for="item in catList4">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="cat_cd_5" v-model="cat_cd_5" style="width: 150px;" @change="changeGbn(event.target)">
										<option value="">- 5차분류 -</option>
										<option :value="item.code" v-for="item in catList5">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="cat_cd_6" v-model="cat_cd_6" style="width: 150px;" @change="changeGbn(event.target)">
							...
							<div class="form-group">
								<label class="control-label" style="float: left;margin-left: 30px;width: 150px;">직종/직급/직렬/경과</label>
								<div style="float: left; margin-left: 20px;">
									<select class="form-control col-sm-1" id="jikgun_cd" v-model="jikgun_cd" style="width: 150px;"@change="changeGbn(event.target)">
										<option value="">- 직종 -</option>
										<option :value="item.code" v-for="item in jikgunList" >{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="jikgub_cd" v-model="jikgub_cd" style="width: 150px;"@change="changeGbn(event.target)">
										<option value="">- 직급 -</option>
										<option :value="item.code" v-for="item in jikgubList">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1" id="pos_cd" v-model="pos_cd" style="width: 150px;"@change="changeGbn(event.target)">
										<option value="">- 직렬 -</option>
										<option :value="item.code" v-for="item in posList">{{item.code_nm}}</option>
									</select>
									<select class="form-control col-sm-1"  id="fdept_cd" v-model="fdept_cd" style="width: 150px;"@change="changeGbn(event.target)">
							...
							<div class="form-group">
								<label class="control-label" style="float: left;margin-left: 30px;width: 150px;">검색어</label>
								<div style="float: left; margin-left: 20px;">
									<input type="text" placeholder="검색어를 입력하세요" style="width: 300px;" v-model="search_val" onkeydown="if(window.event.keyCode==13) return false;" @keyup.enter="getList(true)">	
									<button type="button" class="btn btn-blue btn-icon btn-small" style="width: 90px; float: right; margin-left: 40px;" @click="getList(true)">
										조회
			...
			<div class="form-group" style="margin-bottom: 40px;">
				<div class="pull-left">
					<button type="button" class="btn btn-orange btn-icon icon-left btn-small" @click="addItem">
						추가<i class="entypo-plus"></i>
					</button>
					<button type="button" class="btn btn-red btn-icon icon-left btn-small" @click="deleteList">
						삭제<i class="entypo-trash"></i>
					</button>
					<button type="button" class="btn btn-info btn-icon icon-left btn-small" @click="saveList">
						저장<i class="entypo-box"></i>
					</button>
				</div>
			</div>
			
			<table class="table table-bordered datatable dataTable" style="margin-top: 10px;">
				<thead>
					<tr class="replace-inputs">
						<th style="width: 5px;" class="center nosort">
							<input type="checkbox" id="allCheck" @click="allCheck">
						</th>
						<th class="center" style="width: 22px;">1차 분류</th>
						<th class="center" style="width: 22px;">2차 분류</th>
						<th class="center" style="width: 22px;">3차 분류</th>
						<th class="center" style="width: 22px;">4차 분류</th>
						<th class="center" style="width: 22px;">5차 분류</th>
						<th class="center" style="width: 22px;">6차 분류</th>
						<th class="center" style="width: 22px;">직종</th>
						<th class="center" style="width: 22px;">직급</th>
						<th class="center" style="width: 22px;">직렬</th>
						<th class="center" style="width: 22px;">경과</th>
						<th class="center" style="width: 15px;">기준정원</th>
						<th class="center" style="width: 15px;">운영정원</th>
						<th class="center" style="width: 15px;">배정정원</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item, index) in dataList">
						...
						<td class="center">{{item.cat_nm_1}}</td>
						<td class="center">{{item.cat_nm_2}}</td>
						<td class="center">{{item.cat_nm_3}}</td>
						<td class="center">{{item.cat_nm_4}}</td>
						<td class="center">{{item.cat_nm_5}}</td>
						<td class="center">{{item.cat_nm_6}}</td>
						<td class="center">{{item.jikgun_nm}}</td>
						<td class="center">{{item.jikgub_nm}}</td>
						<td class="center">{{item.pos_nm}}</td>
						<td class="center">{{item.fdept_nm}}</td>
						<td class="center" style="width: 15px;"><input type="text" style="width: 90%;" v-model="item.tot_psn" maxlength="7" v-numform></td>
						<td class="center" style="width: 15px;"><input type="text" style="width: 90%;" v-model="item.now_psn" maxlength="7" v-numform></td>
						<td class="center" style="width: 15px;"><input type="text" style="width: 90%;" v-model="item.assn_psn" maxlength="7" v-numform></td>
					</tr>
				</tbody>
			</table>
